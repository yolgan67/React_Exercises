...
export function newMovies({title,cover}){
  ...
  return(dispatch)=>{
    ...
    // .then(res=>console.log("apı post result",res))})
    .then(res=>res.data)})
  }
}
...
export function deleteMovie(id){
  ...
  return(dispatch)=>{
    dispatch({type: "DELETE_MOVIE",payload:axios.delete(`${API_BASE}/movies/${id}`).then(res=>Object.assign({},res,{id}))})
  }
}
